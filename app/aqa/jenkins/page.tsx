  ...
  return (
    <div className="container">
      ...
      <div className="projectWrapper" style={{ order: 2, width: '100%' }}>
        ...
        <p>You will need set up connection with GitHub via SSH. In short: Jenkins will connect to GitHub &rarr; clone the repository &rarr; build/compile/test this code (depends on settings) &rarr; deploy to the server.</p>
        ...
        <p>Find "Post-build Actions" and click on it &rarr; select "Send build artifact over SSH" &rarr; select the server's name &rarr; in "Source files" type <code>**/*</code>. Run the job. Open your IP in a browser and you will see your site.</p>
        ...
        <p>Find "Launch method" and select "Launch agent via SSH". Field "Host" = server's IP. Credentials &rarr; click "Add" &rarr; select Jenkins &rarr; Field "Kind" select "SSH Username with private key". Type "Username" from your server, usually it's ec2-user and "Private key" &rarr; "Enter directly" &rarr; past your server's key (not the file, but the key data itself). It's better to fill "ID" field and give a recognazible name of this credential.</p> 
        ...
        <p>Create a new job with a type "Freestyle project". In job's settings find "Build Triggers" &rarr; activate the checkbox "Trigger builds remotely (e.g., from scripts)" &rarr; put your token to the field "Authentication Token". Construct URL as shown on Jenkins UI and put in to the browser. You will see nothing in browser, however if open Jenkins UI you will see that this job was started. Example: http://SERVER_IP:8080/job/NAME_OF_JOB/build?token=TOKEN_NUMBER.</p>
        ...
        <p>Create a new job with the type "Pipeline" &rarr; in job's settings find "Pipeline" and add this script</p>
    ...
    stages {
        ...
        <p>Click "Manage Jenkins" &rarr; click "Script Console" and type there commands. For example to change number of executors: <code>Jenkins.instance.setNumExecutors(2)</code></p>
